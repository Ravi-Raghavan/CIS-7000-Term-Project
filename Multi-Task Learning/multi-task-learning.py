...
class SPA(nn.Module):
    ...
    w_s, w_k = softmax(Linear([f_s; f_k]))
    f̃_k = w_s * f_s + w_k * f_k
    """

    def __init__(self, shared_dim: int, private_dim: int):
        super().__init__()
        self.attn = nn.Linear(shared_dim + private_dim, 2)
    ...
    def forward(self, f_s: torch.Tensor, f_k: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            f̃_k: combined features, shape (B, shared_dim + private_dim)
                 (concatenation weighted by learned scalars)
        """
        combined = torch.cat([f_s, f_k], dim=-1)       # (B, D_s + D_k)
        weights = F.softmax(self.attn(combined), dim=-1) # (B, 2)
        w_s = weights[:, 0:1]                            # (B, 1)
        w_k = weights[:, 1:2]                            # (B, 1)

        # Weight each component then concatenate (preserves dimensionality)
        f_combined = torch.cat([w_s * f_s, w_k * f_k], dim=-1)
        return f_combined
...
class SharedEncoder(nn.Module):
    """
    Encodes the concatenation of all private features to capture
    cross-stock (market-level) information.

    Input:  list of K tensors, each (B, private_dim)
    Output: (B, hidden_dim)
    """

    def __init__(self, num_stocks: int, private_dim: int, hidden_dim: int = 64, dropout: float = 0.2):
        super().__init__()
        # Project concatenated private features through a linear layer,
        # then treat as a single-step sequence for the shared LSTM.
        self.proj = nn.Linear(num_stocks * private_dim, hidden_dim)
        self.lstm = nn.LSTM(
    ...
    def forward(self, private_features: list[torch.Tensor]) -> torch.Tensor:
        # private_features: list of K tensors, each (B, private_dim)
        x = torch.cat(private_features, dim=-1)   # (B, K * private_dim)
        x = F.relu(self.proj(x))                  # (B, shared_dim)
        x = x.unsqueeze(1)                         # (B, 1, shared_dim) — single timestep
        _, (h_n, _) = self.lstm(x)
...
class SPAMSJF(nn.Module):
    ...
    def __init__(
        ...
        combined_dim = shared_hidden + private_hidden
        ...
        # Single shared encoder across all stocks
        self.shared_encoder = SharedEncoder(num_stocks, private_hidden, shared_hidden, dropout)

        # One SPA module per stock
        self.spa_modules = nn.ModuleList([
            SPA(shared_hidden, private_hidden)
            for _ in range(num_stocks)
        ])

        # Per-stock task heads
        self.task_heads = nn.ModuleList([
            StockTaskHeads(combined_dim, num_direction_classes)
            for _ in range(num_stocks)
        ])
    ...
    def forward(self, x: torch.Tensor) -> dict:
        ...
        # 2. Shared encoding: sees all private features
        f_s = self.shared_encoder(private_feats)       # (B, shared_hidden)
...
class SlidingWindowTrainer:
    ...
    def evaluate(self, test_dataset: StockDataset) -> dict:
        """Returns per-task predictions and targets for the test window."""
        loader = DataLoader(test_dataset, batch_size=self.batch_size)
        all_preds  = {k: [] for k in ["return", "volatility", "sharpe", "direction", "regime"]}
        all_targets = {k: [] for k in ["return", "volatility", "sharpe", "direction", "regime"]}
        ...
        for x, targets in loader:
            ...
            for k in range(self.model.num_stocks):
                preds_k = outputs["stocks"][k]
                # Accumulate per-stock regression preds
                for task in ["return", "volatility", "sharpe"]:
                    all_preds[task].append(preds_k[task].cpu())
                # Direction: argmax
                all_preds["direction"].append(preds_k["direction"].argmax(-1).cpu())

            all_preds["regime"].append(outputs["regime"].argmax(-1).cpu())

            for k, v in targets.items():
                all_targets[k].append(v.cpu())

        return {
            "predictions": {k: torch.cat(v) for k, v in all_preds.items()},
...
def make_dummy_data(T=500, K=5, input_dim=5):
    """Generate random data for smoke-testing the pipeline."""
    data = np.random.randn(T, K, input_dim).astype(np.float32)
    targets = {
        "return":     np.random.randn(T, K).astype(np.float32),
        "volatility": np.abs(np.random.randn(T, K)).astype(np.float32),
        "sharpe":     np.random.randn(T, K).astype(np.float32),
        "direction":  np.random.randint(0, 3, (T, K)),
        "regime":     np.random.randint(0, 4, (T,)),
    }
    return data, targets
...
if __name__ == "__main__":
    ...
    # ── Dummy data ───────────────────────────────────────
    data, targets = make_dummy_data(T=500, K=5, input_dim=5)

    # Sliding window split: train=6mo (~125 days), val=1mo (~21 days), test=1mo
    SEQ_LEN = 30
    ...
    trainer = SlidingWindowTrainer(
        ...
        device=device,
        lr=1e-3,
        epochs=20,       # increase to 50-100 for real training
        batch_size=32,
        patience=10,
    )
