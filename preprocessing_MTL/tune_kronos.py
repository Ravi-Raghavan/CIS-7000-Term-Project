"""
tune_kronos.py — Smart two-phase hyperparameter search for Kronos + SPA-MSJF
─────────────────────────────────────────────────────────────
...
Results are saved to:
    results/tuning_results.csv   — every trial, every metric
    results/tuning_summary.txt   — human-readable best-config report
...
RESULTS_DIR   = "results"
RESULTS_CSV   = os.path.join(RESULTS_DIR, "tuning_results.csv")
SUMMARY_FILE  = os.path.join(RESULTS_DIR, "tuning_summary.txt")
BEST_CKPT     = os.path.join(RESULTS_DIR, "best_tuned_spa_msjf_kronos.pt")
...
# Tuned specifically for the smaller parameter footprint needed for Kronos (to prevent overfitting)
PHASE1_GRID = {
...
@torch.no_grad()
def evaluate(model, wrapper, loader, device) -> dict:
    model.eval()
    wrapper.eval()
...
def run_trial(
    ...
    model = SPAMSJF(
        num_stocks=5, input_dim=input_dim,
    ...
    # We must reset wrapper gradients/state for each trial (since the tokenizer itself is frozen, only the embeddings inside learn)
    wrapper = wrapper.to(device)
    
    # Reset wrapper trainable params explicitly to ensure a fresh trial
    nn.init.normal_(wrapper.emb_s1.weight, std=0.02)
    nn.init.normal_(wrapper.emb_s2.weight, std=0.02)
    nn.init.xavier_uniform_(wrapper.fusion.weight)
    nn.init.zeros_(wrapper.fusion.bias)
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        model.train()
        wrapper.train()
        wrapper.tokenizer.eval() # Keep tokenizer frozen
        ...
        with torch.no_grad():
            for x, targets in val_dl:
                ...
                x_emb = wrapper(s1, s2)
                _, ld   = criterion(model(x_emb), targets)
                val_loss += ld["total"]
        val_loss /= len(val_dl)
        scheduler.step()
    ...
    wrapper.load_state_dict(best_wrapper_state)
    
    metrics = evaluate(model, wrapper, val_dl, device)
    ...
    return metrics, model, wrapper, best_state, best_wrapper_state
...
def main():
    device = get_device()
    print(f"\n{'='*60}")
    print(f"  Kronos + SPA-MSJF Hyperparameter Tuner")
    ...
    # Shuffle the grid or random subset for faster testing on CPU
    # Just running a subset to prevent out of memory / long compute for user (we take 12 random combos)
    np.random.seed(42)
    np.random.shuffle(p1_grid)
    p1_grid = p1_grid[:12]
    ...
    for i, arch_cfg in enumerate(p1_grid, 1):
        ...
        metrics, model, wrapper, state, wrapper_state = run_trial(cfg, wrapper, input_dim, train_dl, val_dl, device, i, len(p1_grid))

        row = {"phase": 1, **cfg, **metrics}
        save_row(RESULTS_CSV, row, write_header=not csv_header_written)
        csv_header_written = True
        p1_results.append((metrics["score"], cfg, state, wrapper_state))

    # best Phase-1 config (lowest composite score)
    p1_results.sort(key=lambda x: x[0])
    best_p1_score, best_arch_cfg, best_p1_state, best_p1_wrapper_state = p1_results[0]
    ...
    np.random.shuffle(p2_grid)
    p2_grid = p2_grid[:8] # Subset of 8 random lambda configurations
    
    p2_results = []

    for i, lambda_cfg in enumerate(p2_grid, 1):
        cfg = {**best_arch_cfg, **lambda_cfg}
        metrics, model, wrapper, state, wrapper_state = run_trial(cfg, wrapper, input_dim, train_dl, val_dl, device, i, len(p2_grid))

        row = {"phase": 2, **cfg, **metrics}
        save_row(RESULTS_CSV, row, write_header=False)
        p2_results.append((metrics["score"], cfg, model, wrapper, state, wrapper_state))

    p2_results.sort(key=lambda x: x[0])
    best_p2_score, best_cfg, best_model, best_wrapper, best_state, best_wrapper_state = p2_results[0]

    # ── Save best model ───────────────────────────────────────
    torch.save({"model": best_state, "wrapper": best_wrapper_state}, BEST_CKPT)
    ...
    best_wrapper.load_state_dict(best_wrapper_state)
    test_metrics = evaluate(best_model.to(device), best_wrapper.to(device), test_dl, device)

    # ── Summary report ────────────────────────────────────────
    lines = []
    lines.append("=" * 60)
    lines.append("  KRONOS + SPA-MSJF TUNING SUMMARY")
    lines.append("=" * 60)

    lines.append("\n── Phase 1: Top 5 Architecture Configs ──")
    lines.append(f"  {'Score':>8}  {'priv_h':>6}  {'shar_h':>6}  {'spa_d':>5}  {'drop':>5}  {'lr':>8}")
    for score, cfg, _, _ in p1_results[:5]:
        lines.append(
    ...
    for score, cfg, _, _, _, _ in p2_results[:5]:
        lines.append(
